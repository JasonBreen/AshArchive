...
def validate_mod(mod: dict, path: Path, expected_edition: str) -> list[str]:
    ...
    if mod["edition"] != expected_edition:
        errors.append(f"{path}: mod={mod_id} edition '{mod['edition']}' must be '{expected_edition}'")
    if mod["cross_edition_status"] not in CROSS:
        errors.append(f"{path}: mod={mod_id} invalid cross_edition_status '{mod['cross_edition_status']}'")
    ...
    for f in ["plugin_files", "requires", "conflicts", "load_after", "load_before"]:
    ...
    for f in ["name", "category", "source", "url", "archive_name", "version", "patch_notes", "testing_notes", "decision_reason"]:
