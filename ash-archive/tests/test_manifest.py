...
def test_load_meta_document_valid(tmp_path: Path) -> None:
    yaml_file = tmp_path / "valid.yaml"
    yaml_file.write_text("key: value\n", encoding="utf-8")
    assert load_meta_document(yaml_file) == {"key": "value"}


def test_load_meta_document_empty(tmp_path: Path) -> None:
    yaml_file = tmp_path / "empty.yaml"
    yaml_file.write_text("", encoding="utf-8")
    assert load_meta_document(yaml_file) == {}


def test_load_meta_document_missing(tmp_path: Path) -> None:
    missing = tmp_path / "non_existent_file.yaml"
    with pytest.raises(ValueError, match="Missing metadata file"):
        load_meta_document(missing)


def test_load_meta_document_invalid_yaml(tmp_path: Path) -> None:
...
def test_load_meta_document_not_dict(tmp_path: Path) -> None:
...
def test_load_mods_missing_file(tmp_path: Path) -> None:
    missing = tmp_path / "non_existent_file.yaml"
    with pytest.raises(ValueError, match="Missing metadata file"):
        load_mods(missing)


def test_load_mods_invalid_yaml(tmp_path: Path) -> None:
    yaml_file = tmp_path / "invalid.yaml"
    yaml_file.write_text("key: value\n- item", encoding="utf-8")
    with pytest.raises(ValueError, match="Invalid YAML content"):
        load_mods(yaml_file)
